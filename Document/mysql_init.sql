...
create table t_asset (
    ...
    `manual_updated_at` bigint         null        comment '手动更新时间(毫秒)',
...
create table t_order (
    `id`                  bigint         primary key auto_increment comment 'ID',
    `order_remark`        varchar(23)    not null    comment '定单备注(实为下游调用者设定的定单id, 具有唯一性)',
    `account_id`          varchar(32)    not null    comment '账号id',
    `date`                int            not null    comment '日期, format: YYYYMMDD',
    `stock_code`          char(9)        not null    comment '股票代码',
    ...
    `created_at`          timestamp      not null    comment '创建时间戳' default CURRENT_TIMESTAMP,
    `updated_at`          timestamp      not null    comment '更新时间戳' default CURRENT_TIMESTAMP on update CURRENT_TIMESTAMP,
    `manual_updated_at`   bigint         null        comment '手动更新时间(毫秒)',
    `memo`                varchar(255)   null        comment '备注',
